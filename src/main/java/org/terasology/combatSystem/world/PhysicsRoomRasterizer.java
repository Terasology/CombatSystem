...
public class PhysicsRoomRasterizer implements WorldRasterizer {
    ...
    public void generateChunk(Chunk chunk, Region chunkRegion) {
        if (ROOM_CHUNK_SIZE >= chunk.getPosition().x() && chunk.getPosition().x() >= -ROOM_CHUNK_SIZE) {
            if (ROOM_CHUNK_SIZE >= chunk.getPosition().z() && chunk.getPosition().z() >= -ROOM_CHUNK_SIZE) {
                if (chunk.getPosition().y() == 0) {
                    for (int x = CHUNK_REGION.minX(); x <= CHUNK_REGION.maxX(); x++) {
                        for (int z = CHUNK_REGION.minZ(); z <= CHUNK_REGION.maxZ(); z++) {
                            ...
                            if (x == 10 && z == 10) {
                                if (chunk.getPosition().x() == ROOM_CHUNK_SIZE
                                    && chunk.getPosition().z() == ROOM_CHUNK_SIZE) {
    ...
    private boolean isBorder(Chunk chunk, int x, int z) {
        boolean xPos = chunk.getPosition().x() == ROOM_CHUNK_SIZE && x == CHUNK_REGION.maxX();
        boolean xNeg = chunk.getPosition().x() == -ROOM_CHUNK_SIZE && x == CHUNK_REGION.minX();
        boolean zPos = chunk.getPosition().z() == ROOM_CHUNK_SIZE && z == CHUNK_REGION.maxZ();
        boolean zNeg = chunk.getPosition().z() == -ROOM_CHUNK_SIZE && z == CHUNK_REGION.minZ();
